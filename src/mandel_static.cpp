 ...
 *
 * @file mandel_static.cpp
 *
 * @date 20 March 2017
 *
 * @brief A parallel implementation to calculate the Mandelbrot set.
 *        The set is stored as a greyscale image in static.pgm file
 *        and the time to calculate the set is outputted to screen.
 *        This alogrithm implements static load balancing.
 */
//Header Files ////////////////////////////////////////////////////////////////
...
int main( int argc, char *argv[] )
{
  ...
  if( my_rank == MASTER )
  {
    ...
    // testing static paritioning when height divides processors evenly
    totProcs = comm_sz - 1;  /* not including lazy master  */
    nrows = height;
    proc = 1;
    while( nrows > 0 )
    {
      MPI_Send( &row, 1, MPI_INT, proc, tag, MPI_COMM_WORLD );
      if( !( proc % totProcs ) )  /* check if all procs got a new row */
      {
        proc = 0;
      }
      row++;    /* increment to next row to distribute  */
      proc++;   /* increment to next processor          */
      nrows--;  /* decrement total number of rows left  */
    }

    // send kill signal to all processors; no more jobs!
    for( proc = 1; proc < comm_sz; proc++ )
    {
      MPI_Send( &kill, 1, MPI_INT, proc, tag, MPI_COMM_WORLD );
    }

    // receive from any slave and store results
    MPI_Status status;
    for( index = 0; index < height; index++ )
    {
      MPI_Recv( pixels,
      ...
      // store recvd row in 2D image array
